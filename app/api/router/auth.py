...
@auth_router.post(
    ...
    responses={
        ...
        400: {
            "description": f"{FieldMissingException.__name__}: 요청에 필수 필드(code, term_agreements)가 "
            "누락되었거나, 필수 동의 항목에 동의하지 않았거나, 요청 객체에 클라이언트 호스트 정보가 없음"
        },
        422: {
            "description": f"{ValidationException.__name__}: 제출된 약관 동의 정보와 "
            "필요한 약관 정보가 일치하지 않는 등 데이터 유효성 검증에 실패함"
        },
        403: {
            "description": f"{NoPermissionException.__name__}: 요청한 code에 저장된 "
            "호스트 정보와 현재 요청의 호스트가 다르거나 접근 권한이 없음"
        },
        404: {
            "description": f"{DataNotFoundException.__name__}: 요청한 OAuth 인증 코드에 "
            "해당하는 프로필 정보를 캐시 서버에서 찾을 수 없거나, 필요한 약관 정보가 사전에 DB에 존재하지 않음"
        },
        500: {"description": "원인 불명의 내부 로직 오류가 발생함"},
        502: {
            "description": f"{DBServerException.__name__}/{CacheServerException.__name__}: "
            "사용자 정보 저장, 약관 동의 저장, 약관 정보 조회 중 DB 서버 오류가 발생하거나, "
...
@auth_router.get(
    ...
    responses={
        ...
        400: {
            "description": f"{FieldMissingException.__name__}: 요청에 필수 파라미터가 "
            "누락되었거나, 요청 객체에 클라이언트 정보가 없음"
        },
        403: {
            "description": f"{NoPermissionException.__name__}: 요청 호스트와 OAuth 프로필의 "
            "호스트가 일치하지 않음"
        },
        404: {
            "description": f"{DataNotFoundException.__name__}: OAuth 프로필을 찾을 수 없거나 "
            "필요한 약관 정보가 존재하지 않음"
        },
        422: {
            "description": f"{ValidationException.__name__}: 요청 데이터의 형식이 올바르지만 "
            "비즈니스 규칙에 따른 유효성 검증에 실패함"
        },
        500: {"description": "원인 불명의 내부 로직 오류가 발생함"},
        502: {
            "description": f"{CacheServerException.__name__}/{DBServerException.__name__}: "
            "Redis 캐시 서버나 데이터베이스 서버와의 통신 중 오류가 발생함"
        },
    },
)
...
@auth_router.get(
    ...
    responses={
        ...
        422: {
            "description": f"{ValidationException.__name__}: 지원하지 않는 인증 제공자(provider) "
            "값이 전달되었거나, provider 값의 형식이 올바르지 않음"
        },
        500: {"description": "원인 불명의 내부 로직 오류가 발생함"},
        502: {
            "description": f"{OAuthServerException.__name__}: 외부 OAuth 인증 서버와의 "
...
@auth_router.get(
    ...
    responses={
        ...
        403: {
            "description": f"{CSRFException.__name__}: OAuth state 값이 일치하지 않아 "
            "CSRF 공격 가능성이 감지됨"
        },
        422: {
            "description": f"{ValidationException.__name__}: 지원하지 않는 인증 제공자(provider) "
        ...
        500: {"description": "원인 불명의 내부 로직 오류가 발생함"},
        502: {
            "description": f"{OAuthServerException.__name__}/{CacheServerException.__name__}: "
            "외부 OAuth 서버에서 프로필 정보를 가져오는 중 오류가 발생하거나, 프로필 정보를 "
