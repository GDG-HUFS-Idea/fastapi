...
@term_router.get(
    ...
    responses={
        ...
        400: {
            "description": f"{FieldMissingException.__name__}: 요청에 필수 쿼리 파라미터(ids)가 "
            "누락되었거나, 요청된 파라미터의 형식이 올바르지 않음"
        },
        422: {
            "description": f"{ValidationException.__name__}: 요청된 term ID 목록의 형식이 "
            "유효하지 않거나, 허용되지 않는 값이 포함됨"
        },
        404: {
            "description": f"{DataNotFoundException.__name__}: 요청한 약관 ID가 모두 존재하지 않거나, "
            "DB에서 해당 ID에 해당하는 약관 정보를 찾을 수 없음"
        },
        500: {"description": "원인 불명의 내부 로직 오류가 발생함"},
        502: {
            "description": f"{DBServerException.__name__}: 약관 정보 조회 중 DB 서버와의 "
